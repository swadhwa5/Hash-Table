...
//This hash map uses the Double Hashing technique. In this technique, first it
// calculates the index where the
//key should be inserted and checks that position in the table.
// If that position is occupied, it checks
// (hash1(k) + i * hash2(k)) % capacity for i ranging from 0 to capacity - 1.
...
public class HashMap<K, V> implements Map<K, V> {
   ...
   * @return hashed value for the second hash function
   * @throws IllegalArgumentException when k is invalid
   */
  private int hash2(K k, int cap) throws IllegalArgumentException {
    if (k == null) {
      throw new IllegalArgumentException();
    }
    return Math.abs(13 - k.hashCode() % 13);
  }
  ...
  private void doubleHashing(Entry<K, V>[] t, int c, int ind, Entry<K, V> e) {
    for (int i = 0; i < c; i++) {
      int temp = (ind + i * hash2(e.key, c)) % c;
      if (t[temp] == null) {
        t[temp] = e;
        break;
  ...
  public void insert(K k, V v) throws IllegalArgumentException {
    ...
    doubleHashing(table, capacity, index, n);
  ...
  private void grow() {
    ...
    for (int i = 0; i < capacity; i++) {
      if (table[i] != null) {
        ...
        doubleHashing(newTable, newCapacity, newIndex, n);
      }
    }
  ...
  private Entry<K, V> find(K k) throws IllegalArgumentException {
    ...
    while (table[temp] != null) {
      ...
      i++;
      temp = (index + i * hash2(k, capacity)) % capacity;
    }
    return null;
  }
