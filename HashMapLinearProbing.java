...
public class HashMapLinearProbing<K, V> implements Map<K, V> {
  ...
  private void linearProbing(Entry<K, V>[] t, int c, int in, Entry<K, V> e) {
    for (int i = 0; i < c; i++) {
      int temp = (in + i) % c;
      if (t[temp] == null) {
        t[temp] = e;
  ...
  public void insert(K k, V v) throws IllegalArgumentException {
    ...
    linearProbing(table, capacity, index, n);
  ...
  private void grow() {
    ...
    for (int i = 0; i < capacity; i++) {
      if (table[i] != null) {
        ...
        linearProbing(newTable, newCapacity, newIndex, n);
      }
    }
